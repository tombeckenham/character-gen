...
| `character-gen publish <char>`                                                                            | Create/update the character on fal Assets Characters (shells out to the genmedia CLI, which must be on PATH). Sends up to 20 prioritized image request_ids as references + the portrait close-up as the cover; re-publishing updates the existing fal character in place.                                                                                                                   |
...
6. header `Tier` — how much to generate, costs stated plainly in the descriptions (the 12-frame turnaround runs by default on top of every tier — mention it adds 12 generations, skippable):
   - `core` — master + portrait close-up + expression grid + outfit (4 sheet generations; the safe start)
   - `rich` — core + face triptych + 4 named expressions + 2 detail macros (13 sheet generations)
   - `full` — rich + full-body scale shot + up to 4 detail macros (16 sheet generations)
...
```json
{
  ...
  "logline": "A weathered lighthouse keeper who tends a light nobody sails past and collects the last words of storms.",
...
- `logline` (optional but recommended): one crisp sentence summarizing the character. It's published verbatim as the fal Assets `description` (used for semantic search/matching), so make it a real semantic summary, not a visual dump. When absent, publish composes one from archetype/personality/visualCanon.
...
Run `character-gen publish <identifier>` once the character has at least a core sheet (the references are billed fal request_ids — more finished tiers publish richer reference sets, capped at 20 by identity value: portrait/faces > master > scale > expressions > details > cardinal angles > outfits > other angles). The portrait close-up (a core-sheet image) is used as the character's cover, and the `logline` becomes the fal `description`.
